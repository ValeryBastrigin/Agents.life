...
⚠️ **ВАЖНО: НЕ ГАЛЛЮЦИНИРУЙ!** Если в диалоге клиент ничего не сказал (нет его реплик), или сообщения клиента не содержат содержательной информации — напиши честно: «Сеанс был начат, но содержательного диалога не состоялось. Клиент не успел поделиться своими переживаниями.» Не выдумывай темы, эмоции или инсайты, которых не было в диалоге."""
...
async def generate_summary(chat_id: int, db: AsyncSession) -> str:
    ...
    try:
        ...
        # Check if client actually wrote any messages
        client_messages = [m for m in messages if m.role == "user"]
        if not client_messages:
            return "Сеанс был начат, но клиент не успел поделиться своими переживаниями — диалог не состоялся."
