...
const [theme, setTheme] = useState(() => localStorage.getItem('theme') || 'dark');
  ...
  // Edge swipe to open sidebar (like Gemini)
  useEffect(() => {
    let touchStartX = 0;
    let touchStartY = 0;
    ...
    const handleTouchEnd = (e) => {
      if (e.changedTouches && e.changedTouches.length > 0) {
        ...
        // If swipe starts near the left edge (e.g. < 40px) and moves right significantly (> 60px) with minimal vertical movement
        if (touchStartX < 40 && deltaX > 60 && deltaY < 100) {
          setSidebarOpen(true);
        }
      }
    };
...
function AppContent({ theme, sidebarOpen, setSidebarOpen, setTheme }) {
  ...
  return (
      ...
      {location.pathname !== '/terms' && location.pathname !== '/landing' && location.pathname !== '/' && location.pathname !== '/profile' && location.pathname !== '/mentor/tree' && location.pathname !== '/secretary/logs' && location.pathname !== '/login' && !location.pathname.startsWith('/dietitian/plan') && !location.pathname.startsWith('/financial-analyst') && !location.pathname.startsWith('/schedule-manager') && !location.pathname.startsWith('/chat-search') && (
      ...
      <Routes>
        ...
        <Route path="/chat-search" element={<ChatSearch userId={userId} theme={theme} />} />
